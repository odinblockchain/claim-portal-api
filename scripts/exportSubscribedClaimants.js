...
.then(() => {
  ...
  Notification.find({ 'email.myclaim': true })
  .populate('user')
  .exec((err, notificationList) => {
    ...
    notificationListClean = notificationList.map(_notification => {
      return {
        email:    _notification.user.email,
        balance:  _notification.user.balance,
        locked:   _notification.user.balance_locked,
        wallet:   _notification.user.wallet
      };
    });

    const replacer = function(key, value) { return value === null ? '' : value };
    const fields = Object.keys(notificationListClean[0]);

    let csv = notificationListClean.map(function(row) {
    ...
    fs.writeFile(`./export/activeClaimSubscribers-${moment().format('YYYY-MM-DD_X')}.csv`, csv.join('\r\n'), (err) => {
      if (err) {
