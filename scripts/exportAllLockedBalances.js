...
.then(() => {
  ...
  User.find({ balance_locked: true }, { balance_locked_sum: true })
  .populate('user')
  .exec((err, usersList) => {
    if (err) {
      console.log(err);
    ...
    usersListClean = usersList.map(_user => {
      return {
        locked_balance: _user.balance_locked_sum,
      };
    });
    ...
    fs.writeFile(`./export/allLockedBalances-${moment().format('YYYY-MM-DD_X')}.csv`, csv.join('\r\n'), (err) => {
      if (err) {
