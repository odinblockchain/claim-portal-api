    ...
    return new Promise((resolve, reject) => {
      ...
      identity.save((err) => {
        ...
        .then((body) => {
          ...
          if (response.event === 'request.pending' || response.status_code === 'verification.accepted') {
            debug(`Shuftipro::Submission Accepted - user:${user._id}`);
            return resolve({ status: 'ok', event: response.event });
          }
          else {
            debug(`Shuftipro::Signature Accepted - user:${user._id}
            Event:      ${response.event}
            ...
            return resolve({ status: 'error', reason: response.event });
          }
        })
        .catch((err) => {
